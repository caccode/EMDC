function [predY,obj] = EMDC(B,U,V,paras)
% multiview FDC
% Input:
...
for iter = 1:IterMax
    ...
    Bs = full(Bs);
    %% update F
    S=sparse(n+m,n+m);  S(1:n,n+1:end)=P; S(n+1:end,1:n)=P'; S(1:n,1:n)=-D1; S(n+1:end,n+1:end)=-D2;
    y_ind = vec2ind(predY')';
    ...
    %% convergence
    obj(iter) = norm(Bs-P,'fro')^2 - lambda*trace(FF12*(-U'*D1*U-V'*D2*V+2*V'*P'*U));
